...
public class MacroMap extends ScrollView {
  ...
  class Mall {
    class Floor {
      ...
      void drawNavigation(Canvas canvas) {
        ...
        for (int i = 0; i < mNavigation.length(); i++) {
          ...
          x = x * mScale + getWidth() / 2 * (1 - mScale);
          y = y * mScale + getHeight() / 2 * (1 - mScale);
          // if (x < -getWidth() / 3 || x > getWidth() * 4 / 3 || y <
          // -getHeight() / 3 || y > getHeight() * 4 / 3)
          // {
          // return;
          // }
          if (n == 0) {
      ...
      void setOffset(float x, float y) {
        ...
        if (mBorder.width() * mScale <= getWidth()) {
          ...
          // x = Math.min(x, -mBorder.left + getWidth() / 2 -
          // getWidth() / mScale * margin);
      ...
      // if (mBorder != null)
      // {
      // setOffset(x + getWidth() / 2 - mBorder.left, y + getHeight() / 2
      // - mBorder.top);
      // }
      // invalidate();
      // }
      ...
      void setPosition(float x, float y) {
        ...
        // if (mBorder != null)
        // {
        // setOffset(x + getWidth() / 2 / mScale + mBorder.left, y +
        // getHeight() / 2 / mScale + mBorder.top);
        // }
        invalidate();
      }
    ...
    void draw(Canvas canvas) {
      if (mFloor != null) {
        // logd("mFloor.mScale=" + mFloor.mScale + ", mFloor.mOffset=" +
        // mFloor.mOffset + ", canvas=" + canvas);
        Paint paint = new Paint();
  ...
  public boolean onTouchEvent(MotionEvent event) {
    ...
    if (event.getPointerCount() == 1) {
      ...
      if (event.getAction() == MotionEvent.ACTION_DOWN) {
        ...
        changeHighlight(ex - getLeft(), ey - getTop());
        // invalidate();
  ...
  public int setFloor(String id) {
    ...
    for (int i = 0; i < mFloorsAdapter.getCount(); i++) {
      String floorid = mFloorsAdapter.getItem(i).mId;
